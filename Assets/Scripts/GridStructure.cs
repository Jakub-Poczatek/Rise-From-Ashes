...
public class GridStructure
{
    ...
    private void InitialiseStructure(GameObject structure)
    {
        ...
        strSize = new Vector3(
            (Mathf.Ceil(strSize.x) % 2 != 0 && strSize.x > 1) ? Mathf.Ceil(strSize.x): Mathf.Ceil(strSize.x),
            (Mathf.Ceil(strSize.y) % 2 != 0 && strSize.y > 1) ? Mathf.Ceil(strSize.y): Mathf.Ceil(strSize.y),
            (Mathf.Ceil(strSize.z) % 2 != 0 && strSize.z > 1) ? Mathf.Ceil(strSize.z): Mathf.Ceil(strSize.z)
            );
